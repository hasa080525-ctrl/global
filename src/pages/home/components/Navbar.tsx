...
const NAV_LINKS: NavLinkItem[] = [
  { label: "홈", href: "#hero" },
  {
    ...
    dropdownType: "links",
    dropdownItems: [
      { label: "학원 소개", href: "/about", isPage: true },
  ...
  { label: "뉴스", href: "/news", isPage: true, dropdownType: "news" },
  { label: "FAQ", href: "#faq" },
...
export default function Navbar() {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      const insideInfo = infoRef.current?.contains(target);
      if (!insideNews && !insideInfo) {
      ...
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 md:px-6 py-3 md:py-4">
        ...
        <div className="hidden lg:flex items-center gap-1">
          {NAV_LINKS.map((link) => {
            ...
            if (link.dropdownType === "links") {
              return (
                <div key={link.label} className="relative" ref={infoRef}>
                  <button
                    type="button"
                    onClick={() => setOpenDropdown(openDropdown === link.label ? null : link.label)}
                    className="flex items-center gap-1 rounded-full px-3 py-1.5 text-sm font-medium transition whitespace-nowrap cursor-pointer text-background-50 hover:bg-background-50/10"
